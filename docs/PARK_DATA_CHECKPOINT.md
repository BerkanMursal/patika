...
## Milestone: Blocked Source Recovery Sweep (2026-09-24, in progress)
Phase `blocked_source_recovery`. Baseline: HEAD/origin `4ffbd2c`, canonical 26,433, backlog 615. Scope: BLOCKED_CRS -> DOWNLOAD_FAILED -> obvious classifier false-negatives only. NOT: TRULY_NO_STABLE_ID, Manisa. No merge / DB / commit / push. Recovery outputs go to NEW files under `.cache/ulasav/recovery/` (never overwrite `full-classification.json`).
**Next exact step**: enumerate BLOCKED_CRS + DOWNLOAD_FAILED from `.cache/ulasav/full-classification.json`.

### Recovery — family 1: BLOCKED_CRS (2026-09-24)
Both BLOCKED_CRS were mislabelled; neither is a real CRS failure.
- **Gaziantep "Parkomat Listesi ve Konum Bilgisi"** (KML, 198 features, SAFE_OPEN): CRS proven WGS84 (KML declares GEOGCRS WGS 84; extent 37.341–37.797E / 37.010–37.078N = Gaziantep, no reprojection needed). ROOT CAUSE = PARSER ISSUE: geometry is `MultiLineString` which `extractRepresentativePoint()` in inspect-ulasav-full.mjs does not handle -> null -> "0/198 plausible". Independent of CRS. And the dataset is NOT a park inventory: paid on-street parking zones (fields Kapasite, Ucret_Tari, Engelli_Pa, Uzunluk, Yon) — "Parkomat" false positive. Final: not park taxonomy (BLOCKED_TAXONOMY), not usable regardless of the parser fix.
- **Konya/Selçuklu "Selçuklu Belediyesi Parklar"** (CSV, 30 rows, SAFE_OPEN via Konya CC BY 4.0 portal): ROOT CAUSE = swapped axis LABELS (no CRS metadata in CSV): column `enlem` (latitude) holds 32.40–32.53 and `boylam` (longitude) holds 37.89–38.01; as (lat=enlem, lon=boylam) 0/30 plausible for Turkey (lat 35.8–42.2), as (lat=boylam, lon=enlem) 30/30. Proven independently (not merely "lands in Turkey"): 14 rows name-match same-named parks in Konya's first-party integrated dataset; swapped reading lands within 50 m for 9, within 150 m for 12 (2 outliers are distant namesakes), while the as-labelled reading is ~780 km away for all 14 (`.cache/ulasav/recovery/selcuklu/axis-swap-evidence.json`). CRS after fix = WGS84 lat/lon (axis-swap only). BUT: only {park_adi, enlem, boylam}, no id field -> BLOCKED_IDENTITY (name/coordinate never usable as id); also a 30-row subset of the already-integrated Konya dataset. Final: BLOCKED_IDENTITY (CRS recovered).
BLOCKED_CRS totals: investigated 2, CRS/geometry cause recovered 2, READY 0, still blocked 2 (different blockers: taxonomy, identity).
**Next exact step**: DOWNLOAD_FAILED family (5): Arnavutköy GeoJSON, Balıkesir Değirmenboğazı KML, Manisa `parklar.kml` + Mutlu KMZ (root-cause only; Manisa stays BLOCKED_LICENSE), Mersin Yenişehir SHP.

### Recovery — family 2: DOWNLOAD_FAILED root causes (2026-09-24)
5 datasets investigated (raw files in `.cache/ulasav/recovery/dl/`):
- **İstanbul-Arnavutköy "Arnavutköy Park Yerleri"** (GeoJSON): host `acikveri.arnavutkoy.bel.tr` = NXDOMAIN (dead portal). ULASAV siblings (XLSX + GeoJSON) are on the same dead host; `www.arnavutkoy.bel.tr` is alive but links to no open-data portal; no official successor found. Cause = DEAD_SOURCE. Not recoverable (no unofficial mirrors used).
- **Mersin-Yenişehir "Rekreasyon Alanları"** (SHP zip): `acikveri.yenisehir.bel.tr` resolves (212.174.1.242) but times out on 443 and 80 while `www.yenisehir.bel.tr` returns 200; no official replacement linked. Cause = PORTAL_HOST_DOWN (not transient across the session). Not recoverable.
- **Balıkesir "Değirmenboğazı Tabiat Parkı yürüyüş güzergahı"** (KML, 200 OK, 23.7 KB): ogr2ogr error "Layer 'Kahverengi Rota' does not already exist in the output dataset" = PARSER ISSUE: multi-layer KML (one OGR layer per Folder) cannot be written to a single GeoJSON. Content is a hiking route (nature-park trail), not a park inventory.
- **Manisa "Parklar"** (`parklar.kml`, 200 OK, 180 KB): same multi-layer parser issue ("Layer 'PARKLAR (#2)'"). Manisa = root cause only; stays BLOCKED_LICENSE.
- **Manisa "Mutlu Mahallesinde Bulunan Parklar"** (KMZ, 363 B): archive OK, but doc.kml (418 B) contains only an EMPTY folder "Geçici Yerler" (0 placemarks) -> ogr2ogr writes an empty file -> "Unexpected end of JSON input". Cause = EMPTY_SOURCE + inspector crash on empty output. Nothing to recover.
Also found: the inspector labels 40 datasets (36 Manisa, Arnavutköy, Sivas, Mersin-Yenişehir) SAFE_OPEN solely from the ULASAV `<city>-cc-by` id pattern — proven boilerplate (see Manisa licence milestone). To be corrected in the inspector.
**Next exact step**: patch scripts/inspect-ulasav-full.mjs (MultiLineString/MultiPoint points; multi-layer + empty-output loader fallback; id coverage over valid-geometry rows + property-over-feature.id tie-break; drop the unverified CC-BY pattern; optional per-resource CSV axis override) + runner `--output/--only`, then regression-diff all 71 into a NEW file.

### Recovery — inspector fixes + targeted re-run (2026-09-24)
Patched `scripts/inspect-ulasav-full.mjs` (generic): (1) representative point for MultiLineString / MultiPoint / GeometryCollection; (2) multi-layer KML fallback (per-layer ogr2ogr, layer kept in `__layer`) and EMPTY output = 0 features instead of a crash — only reached after the single-file conversion fails, so working datasets are unaffected; (3) identity judged over VALID-GEOMETRY rows (null ids on un-ingestable rows no longer poison a complete field) + when exactly one named property ties with GeoJSON `feature.id` the property wins; (4) the ULASAV `<city>-cc-by` "CC BY" pattern no longer yields SAFE_OPEN (-> LICENSE_UNVERIFIED); (5) optional per-resource evidence-backed CSV axis override via new `data/ulasav-recovery-overrides.json` (only Selçuklu). `scripts/run-ulasav-full-classification.mjs` gained `--output=` / `--only=` (defaults unchanged, never overwrites full-classification.json).
Targeted re-run (`.cache/ulasav/recovery/targets-run.json`, `balikesir-run.json`):
- Gaziantep Parkomat -> NOT_PARK_DATA (198/198 valid geometry, WGS84, in province, id `id` 198/198; parking, not parks).
- Konya/Selçuklu CSV -> BLOCKED_IDENTITY (30/30 valid after proven axis-swap, 30/30 in Konya, 29 name-inferred PARK, no id field).
- Ordu Büyükşehir Parkları -> READY_FOR_RECONCILIATION (FALSE-NEGATIVE FIXED: ID 208/208 over valid-geometry rows) — but ALREADY INTEGRATED as `ordu_acikveri_parklari`, so NOT a new source.
- Konya "Parklar" -> identity fixed (POI_ID 2138/2138; was tie with feature.id), READY_AFTER_SMALL_PARSER only because generic taxonomy whitelist lacks ALT_NITELIK_ADI (title-only) — ALREADY INTEGRATED as `konya_acikveri_parklar` (adapter verifies ALT_NITELIK_ADI=='PARK').
- Balıkesir Değirmenboğazı -> loads now (6 route features, 6/6 in province) -> BLOCKED_IDENTITY (no id; trail routes, not a park inventory).
- Manisa Parklar + Mutlu, Arnavutköy, Mersin-Yenişehir -> BLOCKED_LICENSE (license gate runs before download; LICENSE_UNVERIFIED after fix 4). Manisa parklar.kml = 4+ layers (PARKLAR, PARKLAR (#2), ADİLOBA, YILMAZ...) so the multi-layer fix applies; Manisa still BLOCKED_LICENSE.
**Next exact step**: full 71-dataset regression rerun with the fixed inspector into `.cache/ulasav/recovery/full-classification-v3.json`, diff vs `full-classification.json`; verify canonical 26,433 / backlog 615 untouched.

### Blocked source recovery sweep — FINAL (2026-09-24) — STOPPED, nothing merged/committed
Regression: full 71-dataset rerun with the fixed inspector -> `.cache/ulasav/recovery/full-classification-v3.json` (original `full-classification.json` untouched). 44 status changes, ALL explained: 34 Manisa + Sivas BLOCKED_IDENTITY -> BLOCKED_LICENSE (unverified "CC BY" pattern no longer SAFE_OPEN); 4 DOWNLOAD_FAILED -> BLOCKED_LICENSE (Arnavutköy, Manisa x2, Mersin-Yenişehir; license gate precedes download); Balıkesir Değirmenboğazı DOWNLOAD_FAILED -> BLOCKED_IDENTITY (multi-layer fix); Gaziantep BLOCKED_CRS -> NOT_PARK_DATA (MultiLineString fix); Selçuklu BLOCKED_CRS -> BLOCKED_IDENTITY (axis override); Konya Parklar BLOCKED_SCHEMA -> READY_AFTER_SMALL_PARSER (tie-break; already integrated); Ordu BLOCKED_IDENTITY -> READY_FOR_RECONCILIATION (null-row false negative; already integrated). 0 field diffs where status unchanged. v3 totals: READY 4 (Kayseri-Kocasinan, Van, Trabzon — all already integrated — plus Ordu, already integrated), BLOCKED_LICENSE 53, BLOCKED_IDENTITY 11, READY_AFTER_SMALL_PARSER 2 (Bursa tourism list, Konya Parklar), NOT_PARK_DATA 1.
**BLOCKED_CRS**: investigated 2, recovered (root cause proven + fixed) 2, READY 0 — Gaziantep Parkomat: CRS was fine (WGS84), real cause MultiLineString parser gap; parking data -> NOT_PARK_DATA. Konya/Selçuklu: swapped axis labels, proven with first-party evidence; 30 rows, no id -> BLOCKED_IDENTITY.
**DOWNLOAD_FAILED**: investigated 5, technically recovered 2 (Balıkesir Değirmenboğazı multi-layer -> now BLOCKED_IDENTITY; Manisa `parklar.kml` multi-layer parse fixed but BLOCKED_LICENSE), still failed/unusable 3: Arnavutköy = DEAD_SOURCE (NXDOMAIN, no official successor), Mersin-Yenişehir = PORTAL_HOST_DOWN (timeout, no official successor), Manisa Mutlu KMZ = EMPTY_SOURCE (0 placemarks). No mirrors used.
**Classifier false negatives corrected: 2** (Ordu ID null-row poisoning, Konya POI_ID vs feature.id tie) — both already-integrated sources, so no new sources; plus 1 false POSITIVE class fixed (ULASAV `<city>-cc-by` label -> SAFE_OPEN, 40 datasets). The 14 BLOCKED_LICENSE (Kocaeli, Kırıkkale x8, Tuzla x3) re-checked: genuinely unverified (Tuzla registry match is İBB, a different entity; Kocaeli recorded UNUSABLE) -> no false negatives.
**NEWLY RECOVERED READY_FOR_RECONCILIATION: NONE** (0 new sources). Only Ordu flipped to READY and it is already integrated (`ordu_acikveri_parklari`): Ordu / Ordu Büyükşehir Belediyesi / "Ordu Büyükşehir Parkları" / GeoJSON / 210 raw / 149 PARK candidates (name-inferred) / 208 valid / ID 208/208 / WGS84 (GeoJSON default) / SAFE_OPEN (municipal-park-sources.json).
Files changed (uncommitted): M scripts/inspect-ulasav-full.mjs, M scripts/run-ulasav-full-classification.mjs, ?? data/ulasav-recovery-overrides.json, M docs/PARK_DATA_CHECKPOINT.md, M data/park-enrichment/progress.json. Regression: canonical 26,433 / OSM 24,750 / backlog 615 / 4 merged sources+Kayseri+Van unchanged, canonical preview mtime unchanged; no DB, no merge, no commit/push.
